...
> [Release 说明](https://github.com/qgx1992/DSH-Exoskeleton/releases/tag/v0.9.5) · 完整历史见 [更新日志](CHANGELOG.md)

- **修复「通知里的会话名和侧边栏对不上」**：AI 会自动给会话重命名，而壳侧取的是**重命名前**的临时标题（内核口径是取**最后一条** `session/title`，壳侧取了第一条）。表现为界面显示「修复右键插件」、通知却显示「dsh中安装的dsh-session-context-menu…」。实测 **88%（53/60）** 的会话首尾标题不同。现已与内核口径对齐，修复后 66/66 个会话完全一致。
- **通知正文补上「本次提问」，多轮会话不再长得一模一样**：会话名是整个会话的名字，所以一个会话里无论第几轮完成，弹出来的第一行都一样——看得出「哪个会话」，看不出「刚完成哪一轮」。现正文改为两行：第一行「会话名（第 N 轮）」，第二行「本次：<本轮真实提问>」。难点是「本轮提问」没有现成字段（`user/message` 不带轮次编号，且真实日志里一条轮次后面常跟着多条**系统注入的伪用户消息**如运行时上下文/技能提示），已按实测前缀白名单过滤，实测 205 个已完成轮次命中率 **98%**。
- **修复部分会话通知标题退化成 uuid**：取标题时只读前 512KB，而 zstd 是按批压缩的——一次超长输出可能压成单帧上 MB（实测有 1011KB 的单帧正好带着标题），该帧被整段截在缓冲区外，标题就成了 `会话 dcfea798`。实测 **12%（13/105）** 会话受影响。现改为按字节渐进扩读。
- **修复「会话完成通知整体失效」**（内核升到 Session format v3 后通知和会话列表都不再工作）：DSH 内核 0.1.5 起把会话日志从 `session.jsonl.zstd` 改为**代际化命名** `session.v3.jsonl.zstd`，而壳侧把文件名写成了字面量，于是找不到文件 → 会话完成 / 询问卡等待通知**全部静默丢失**（无任何报错，只是什么都不弹），管理面板里升级后的会话也看不见。实测本机 90 个会话只列得出 40 个。现改为按内核的世代规则**取最高代数**定位日志（v0/v3 并存时读 v3），通知与列表一并恢复。
- **修复 `npm run clean:dist` 从不清理 beta 产物**：旧正则认不出 `0.9.4-beta.4.exe` 这类名字 → 产物只增不减（实测累积到 1.4GB）。现按「正式版 / 预发布版」两条通道分别保留，并修正版本排序（`beta.10` 不再排在 `beta.2` 前面）。
