...
> [Release 说明](https://github.com/qgx1992/DSH-Exoskeleton/releases/tag/v0.9.3) · 完整历史见 [更新日志](CHANGELOG.md)

- **修复内核「卸载不了」**：内核的原生模块（sharp / koffi）在多个版本之间是 pnpm 硬链接共享的**同一个文件对象**，被运行中的内核进程映射后 Windows 会拒绝删除。旧实现在删到一半时中断，会把内核删成损坏状态却仍显示「已安装」，而且每点一次卸载就再破坏一次。现在改为**改名优先、删除兜底**：卸载即时生效，物理删除交给后台重试，删不掉也不破坏文件，留到下次启动回收。
- **新增启动对账**：自动识别「文件已损坏」「安装被中断」的内核残留并标记为「已损坏」，可一键清理——此前这类残留在界面上完全看不见却一直占盘（实测泄漏 252MB）；同时清理指向失效内核的默认内核指针与崩溃回滚指针。
- **反馈更可操作**：占用类报错（`EPERM` / `EBUSY`）翻译成中文并给出处理建议；内核占用按磁盘实测统计，并单列「待回收」。
- **启动不再被对账拖慢**：损坏内核的占用重测改为后台异步执行（此前同步扫描大目录会卡住启动数秒）。
