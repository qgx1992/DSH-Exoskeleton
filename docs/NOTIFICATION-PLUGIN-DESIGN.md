...
auto 决策    = webview 在线 && notifyChannel=auto → webview
              否则 → native（→ 托盘降级链）
```
...
## 6. DSH 插件侧：dsh-notify（新仓库，或并入 dsh-ui-tools）

### 6.1 插件形态

- **归属：本地插件，默认不上传 GitHub / 不发布 npm**（源码在壳仓库 `plugins/dsh-notify/`，安装走 `dsh plugin --profile web add link:<本地路径>`）；若后续公开分发再补 `github:qgx1992/dsh-notify` + npm 双源；
