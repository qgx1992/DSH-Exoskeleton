...
- **Fixed “session-completion notifications stopped working entirely”** (after the kernel moved to Session format v3, both notifications and the session list went dark): since DSH kernel 0.1.5 the session log is named by **format generation** (`session.v3.jsonl.zstd`) instead of `session.jsonl.zstd`, but the shell hard-coded the old filename, so the file was never found → session-completion and ask-card notifications were **silently dropped** (no error at all, just nothing popping up), and sessions created after the upgrade were invisible in the management panel. On this machine only 40 of 90 sessions were listed. The shell now resolves the log by the kernel's own generation rule, **always taking the highest generation** (v3 when v0 and v3 coexist), which restores both notifications and the session list.
