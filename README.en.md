...
## Latest Release Highlights (v0.9.5)

> [Release notes](https://github.com/qgx1992/DSH-Exoskeleton/releases/tag/v0.9.5) · Full history in the [changelog](CHANGELOG.md)

- **Fixed “the session name in notifications does not match the sidebar”**: the AI renames sessions automatically, but the shell was reading the **pre-rename** temporary title (the kernel reads the **last** `session/title`, the shell read the first one). So the sidebar showed “Fix right-click plugin” while the notification showed “dsh中安装的dsh-session-context-menu…”. Measured: **88% (53/60)** of sessions had different first/last titles. Now aligned with the kernel — 66/66 sessions match exactly after the fix.
- **Notifications now include “this turn's question”, so multi-turn sessions no longer look identical**: the session name is the name of the whole session, so the first line was the same no matter which turn finished — you could tell *which session* but not *which turn*. The body is now two lines: “session name (turn N)” and “本次: <the actual question of this turn>”. The hard part is that “this turn's question” has no ready-made field (`user/message` carries no turn number, and a turn is usually followed by several **system-injected pseudo user messages** such as runtime context or skill hints), so they are filtered by a measured prefix allowlist — 98% hit rate across 205 completed turns.
- **Fixed notification titles degrading to a UUID for some sessions**: only the first 512KB was read, but zstd compresses per append batch — one very long output can become a single multi-MB frame (a real 1011KB frame carried the title), and that frame fell outside the buffer, leaving the title as `会话 dcfea798`. Measured **12% (13/105)** of sessions affected. Reads now grow progressively by bytes.
- **Fixed “session-completion notifications stopped working entirely”** (after the kernel moved to Session format v3, both notifications and the session list went dark): since DSH kernel 0.1.5 the session log is named by **format generation** (`session.v3.jsonl.zstd`), but the shell hard-coded the old filename, so the file was never found → session-completion and ask-card notifications were **silently dropped** and sessions created after the upgrade were invisible in the management panel. On this machine only 40 of 90 sessions were listed. The shell now resolves the log by the kernel's own generation rule, **always taking the highest generation**, which restores both notifications and the session list.
- **Fixed `npm run clean:dist` never pruning beta artifacts**: the old regex did not recognise names like `0.9.4-beta.4.exe`, so `dist/` only ever grew (measured at 1.4GB before anyone noticed). Formal and pre-release channels are now pruned separately, and version ordering was corrected (`beta.10` no longer sorts before `beta.2`).
