...
- 通知标题为「项目名 · DSH 对话完成」（无项目名时仅基础标题），正文为会话名 + 轮次（第 N 轮），第二行为「本次：本轮提问」
  - v0.9.5 调整：项目名原在正文开头（`项目「X」· 标题`），与会话标题（本身就是用户首句提问）叠在一起很快被截断；现移至标题行，正文腾出可见长度
  - **会话名必须取最后一个 `session/title`（否则与 UI 不一致）**：一个会话的 title 事件序列实测是「截断的首句提问（临时）→ `title-llm-request`（带请求无 title）→ AI 精炼后的正式名字」，**内核口径**是 `dsh-client-connection` 的 `log.findLast(item => item.type === 'session/title')`。壳侧若取第一条，通知会显示**重命名前的临时标题**（实测 53/60 = 88% 会话首尾不同），与 DSH 侧边栏显示的会话名对不上。同理：取「最后一个」意味着不能一拿到 title 就提前停读，需扫到足够晚的位置（实测 AI 重命名在首轮内、最大偏移 1.01MB，故最低扫 2MB、封顶 8MB）。
  - 为何要「本次：」：会话名是整个会话的名字（AI 重命名后通常很概括，如「修复右键插件」），多轮会话的每条通知第一行会一样——看不出刚完成的是哪一轮。第二行补上本轮真实提问才能区分。
  - 「本轮提问」的取法（`zstd-worker.cjs`）：`user/message` 事件**不带 turn 编号**，只能按顺序关联——每遇 `turn/start` 切换当前轮，该轮内**第一条非注入**的 user 消息即本轮提问。必须过滤系统注入的伪用户消息（`Current runtime context` / `<system-reminder>` / `[genui-action]` / `[model changed` / `[Suggestion mode` / `The user …` / `background job`），否则会把运行时上下文当成人问的话。实测 65 个真实会话 / 205 个已完成轮次，命中率 98%（未命中则回落为只显示会话名，不拼空的「本次：」）。
